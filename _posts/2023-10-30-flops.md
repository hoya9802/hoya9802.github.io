...
### Pooling
첫번째 방법은 Pooling를 사용하는 것입니다. 아래 사진은 Pooling이 작동하는 방식으로 해당 Pooling를 사용하게 되면 이로 인해 Feature map이 Input보다 작아지면서 그로 인해 FLOPs도 감소하게 됩니다. 또한 Pooling은 학습 가능한 파라미터가 없기 때문에 Pooling 연산 자체는 weight와 bias와 관련이 없으므로 FLOPs를 줄이는 데 도움이 됩니다.<br>
